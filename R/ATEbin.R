# Binary ATE with ps and outcome model specified


# module for calculating the estimates
ATEbin <- function(ps.formula,ps.estimate=NULL,zname=NULL,yname,data,trtgrp=NULL,augmentation=FALSE,bootstrap=FALSE,R=200,out.formula=NULL,out.estimate=NULL,family=NULL,delta=0){
  ...
  if(augmentation==FALSE&bootstrap==TRUE){
      ...
      for(i in 1:R){
        ...
        # point estimate
        mu1.b <- sum(z.b*y.b/e.b) / sum(z.b/e.b)
        mu0.b <- sum((1-z.b)*y.b/(1-e.b)) / sum((1-z.b)/(1-e.b))
  ...
  if(augmentation==TRUE){
      ...
      aug0h.h<-mean(m0.h)
      aug0z.h<-sum((1-z)*m0.h/(1-e.h))/sum((1-z)/(1-e.h))
      aug1h.h<-mean(m1.h)
      aug1z.h<-sum(z*m1.h/e.h)/sum(z/e.h)
      ...
      if(bootstrap==FALSE){
        ...
        if(family=='gaussian'){
          for(i in 1:R){
            ...
            aug0h.b<-mean(m0.b)
            aug0z.b<-sum((1-z.b)*m0.b/(1-e.b))/sum((1-z.b)/(1-e.b))
            aug1h.b<-mean(m1.b)
            aug1z.b<-sum(z.b*m1.b/e.b)/sum(z.b/e.b)
