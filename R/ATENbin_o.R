# Binary ATEN(entropy) with outcome value supplied


ATENbin_o <- function(ps.formula,ps.estimate=NULL,zname=NULL,yname,data,trtgrp=NULL,augmentation=FALSE,bootstrap=FALSE,R=200,out.formula=NULL,out.estimate=NULL,family=NULL,delta=0){
  ...
  phi<-function(theta){
    ...
    f1<-z*htilt/e*(y-mu1)
    f2<-(m1.h-aug1h)*htilt
    f3<-z*htilt/e*(m1.h-aug1z)
    f4<-(1-z)*htilt/(1-e)*(y-mu0)
    f5<-(m0.h-aug0h)*htilt
    f6<-(1-z)*htilt/(1-e)*(m0.h-aug0z)
    f7<-W*(z-e)
