# Binary ATT with both ps and outcome model specified



ATTbin <- function(ps.formula,ps.estimate=NULL,zname=NULL,yname,data,trtgrp=NULL,augmentation=FALSE,bootstrap=FALSE,R=200,out.formula=NULL,out.estimate=NULL,family=NULL,delta=0){
  ...
  if(augmentation==FALSE&bootstrap==TRUE){
    ...
    for(i in 1:R){
      ...
      # point estimate
      mu1.b <- sum(z.b*y.b) / sum(z.b)
      mu0.b <- sum((1-z.b)*y.b*e.b/(1-e.b)) / sum((1-z.b)*e.b/(1-e.b))
      tau.b<-c(tau.b,mu1.b-mu0.b)
  ...
  #augmentation, no bootstrap, compute the augmented tau and variance
  if(augmentation==TRUE&bootstrap==FALSE){
    ...
    phi<-function(theta){
      ...
      if (family=='gaussian'){
        ...
        m1<-c(plogis(c(XY%*%gamma1)))
        m0<-c(plogis(c(XY%*%gamma0)))
  ...
  if(augmentation==TRUE&bootstrap==TRUE){
    ...
    if(family=='gaussian'){
      for(i in 1:R){
        ...
        aug0h.b<-sum(m0.b*e.b)/sum(e.b)
        aug0z.b<-sum((1-z.b)*m0.b*e.b/(1-e.b))/sum((1-z.b)*e.b/(1-e.b))
        aug1h.b<-sum(m1.b*e.b)/sum(e.b)
        aug1z.b<-sum(z.b*m1.b)/sum(z.b)
