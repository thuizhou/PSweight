# Binary ATM with ps and outcome model specified


# module for calculating the estimates
ATMbin <- function(ps.formula,ps.estimate=NULL,zname=NULL,yname,data,trtgrp=NULL,augmentation=FALSE,bootstrap=FALSE,R=200,out.formula=NULL,out.estimate=NULL,family=NULL,delta=0){
  ...
  if(augmentation==FALSE&bootstrap==FALSE){
      ...
      ##since tilting function nondifferentiable, we do not consider its variability
  ...
  if(augmentation==FALSE&bootstrap==TRUE){
      ...
      for(i in 1:R){
        ...
        htilt.b<-pmin(e.b,1-e.b)

        # point estimate
        mu1.b <- sum(z.b*y.b*htilt.b/e.b) / sum(z.b*htilt.b/e.b)
        mu0.b <- sum((1-z.b)*htilt.b*y.b/(1-e.b)) / sum((1-z.b)*htilt.b/(1-e.b))
  ...
  if(augmentation==TRUE&bootstrap==FALSE){
      ...
      phi<-function(theta){
        ...
        f2<-htilt.h*(m1-aug1h)
        f3<-z*htilt.h/e.h*(m1-aug1z)
        f4<-(1-z)*htilt.h/(1-e.h)*(y-mu0)
        f5<-htilt.h*(m0-aug0h)
        f6<-(1-z)*htilt.h/(1-e.h)*(m0-aug0z)
        f7<-XY*(y-m1)*z
        f8<-XY*(y-m0)*(1-z)
