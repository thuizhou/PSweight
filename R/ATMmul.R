# Multiple ATM with ps and outcome model specified


ATMmul<-function(ps.formula,ps.estimate=NULL,zname=NULL,yname,data,trtgrp=NULL,augmentation=FALSE,bootstrap=FALSE,R=200,out.formula=NULL,out.estimate=NULL,family=NULL,delta=0){
  ...
  #use the minimun propensity to get the weight
  htilt.h<-apply(e.h, 1, min)
  allwt<-(1/e.h)*htilt.h
  wt<-rep(0,n)
  for(i in 1:ncate){
    wt[z==i]<-allwt[z==i,i]
