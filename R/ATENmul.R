# Multiple ATEN(entropy) with ps and outcome model specified


ATENmul<-function(ps.formula,ps.estimate=NULL,zname=NULL,yname,data,trtgrp=NULL,augmentation=FALSE,bootstrap=FALSE,R=200,out.formula=NULL,out.estimate=NULL,family=NULL,delta=0){
  ...
  #use the entropy to get the weight
  e.hclip<- pmax(e.h,1e-6)
  htilt.h<-(-apply(e.hclip*log(e.hclip) ,1,sum))
  allwt<-(1/e.h)*htilt.h
  wt<-rep(0,n)
  for(i in 1:ncate){
    wt[z==i]<-allwt[z==i,i]
  ...
  if(augmentation==FALSE&bootstrap==FALSE){
    ...
    if(conser==1){
      phi<-function(theta){
        mu<-theta[1:ncate]
        wtt<-(1/e.h)*(-apply(e.h*log(e.h) ,1,sum))
        fmu<-c()
        fbeta<-c()
        for(i in 1:ncate){
          fmutmp<-(y-mu[i])*wtt[,i]*(z==i)
  ...
  if(augmentation==FALSE&bootstrap==TRUE){
    ...
    for(i in 1:R){
      ...
      allwt.b<-(1/e.b)*(-apply(e.clip.b*log(e.clip.b) ,1,sum))
      #the point estimate
      mu.tmp<-c()
      for(i in 1:ncate){
        ytmp.b<-y.b[z.b==i]
